...
namespace Cotizador.ExcelExport
{
    ...
    public class ReporteCargaStock
    {
        ...
        public FileStreamResult generateExcel(List<RegistroCargaStock> list, Usuario usuario)
        {
            ...
            foreach (RegistroCargaStock obj in list)
            {
                if (!sede.Equals(obj.ciudad.nombre))
                {
                    ...
                    sheet = NuevaHoja(wb, sede, list.Count + 200);
                    i = 3;
                    familia = "";
                }
                ...
                //decimal stockProv = obj.cantidadConteo/(obj.producto.equivalenciaProveedor*obj.producto.equivalenciaUnidadEstandarUnidadConteo);
                decimal stockProv = (decimal) obj.cantidadProveedor;
                if (obj.cantidadMp > 0) {
                    stockProv = stockProv + (((decimal)obj.cantidadMp) / ((decimal) obj.producto.equivalenciaProveedor));
                }

                if (obj.cantidadAlternativa > 0)
                {
                    stockProv = stockProv + (((decimal)obj.cantidadAlternativa) / ((decimal)(obj.producto.equivalenciaProveedor*obj.producto.equivalenciaAlternativa)));
                }
                ...
                if (bStyle)
                {
                    ...
                    UtilesHelper.setValorCelda(sheet, i, "G", String.Format(Constantes.formatoDosDecimales, stockProv), tableDataCenterCellStyleB);
                ...
                else
                {
                    ...
                    UtilesHelper.setValorCelda(sheet, i, "G", String.Format(Constantes.formatoDosDecimales, stockProv), tableDataCenterCellStyle);
        ...
        private HSSFSheet NuevaHoja(HSSFWorkbook wb, string titulo, int rows)
        {
