...
jQuery(function ($) {
    ...
    $("#btnFinalizarCreacionNotaCredito").click(function () {
        var sustento = $("#compra_sustento").val();
        ...
        if (convertirFechaNumero($("#documentoVenta_fechaEmision").val()) < convertirFechaNumero($("#compra_documentoReferencia_fechaEmisionFormat").val())) {
            alert("La fecha de emisión de la nota de crédito no puede ser anterior a la fecha de emisión de la factura.");
            $("#documentoVenta_fechaEmision").focus();
            return false;
        }

        var serie = $("#compra_documentoCompra_serie").val();
        var numero = $("#compra_documentoCompra_numero").val();

        serie = serie.trim();
        if (serie.length < 1) {
            alert("Debe ingresar la serie de la nota de crédito.");
            $("#compra_documentoCompra_serie").focus();
            return false;
        }

        numero = numero.trim();
        if (numero.length < 1) {
            alert("Debe ingresar el número de la nota de crédito.");
            $("#compra_documentoCompra_numero").focus();
            return false;
        }
        ...
        var observaciones = $("#compra_observaciones").val();
        ...
        $.ajax({
            ...
            success: function (documentoCompra) {
                ...
                if (documentoCompra.cPE_RESPUESTA_COMPRA.CODIGO == "001") {
                    ...
                    $("#vpDIR_DES_RCT").html(documentoCompra.cPE_RESPUESTA_COMPRA.DIR_DES_RCT);
                    $("#vpNRO_ORD_COM").html(documentoCompra.cPE_RESPUESTA_COMPRA.NRO_ORD_COM);

                    /*RUC - NRO GUIA*/
                    $("#vpNRO_DOC_RCT").html(documentoCompra.cPE_RESPUESTA_COMPRA.NRO_DOC_RCT);
                    $("#vpNRO_GRE").html(documentoCompra.cPE_RESPUESTA_COMPRA.NRO_GRE);

                    /*OBSERVACIONES*/ /*CODIGO CLIENTE*/
                    $("#vpOBSERVACIONES").html($("#compra_observaciones").val());
    ...
    $("#btnConfirmarGeneracionNotaCredito").click(function () {
        ...
        $.ajax({
            url: "/NotaCreditoCompra/ConfirmarCreacion",
            type: 'POST',
            dataType: 'JSON',
            data: {
  ...
  /*      var $j_object1 = $("td.detporcentajedescuento");
    ...
    $(document).on('click', "button.footable-hide", function () {
        ...
        $.ajax({
            url: "/NotaCreditoCompra/ChangeDetalle",
            type: 'POST',
            data: json,
            dataType: 'json',
    ...
    $('#documentoVenta_observacionesUsoInterno').change(function () {
        ...
        $.ajax({
            url: "/NotaCreditoCompra/ChangeObservacionesUsoInterno",
            type: 'POST',
            data: {
