...
jQuery(function ($) {
    ...
    function validacionDatosOrigen() {
    ...
    $("#origen_estado_si").click(function () {
        var valCheck = 1;
        changeInputInt("Estado", valCheck)
    });

    $("#origen_estado_no").click(function () {
        var valCheck = 0;
        changeInputInt("Estado", valCheck)
    });
    ...
    $("#origen_codigo").change(function () {
        changeInputString("codigo", $("#origen_codigo").val());
    });

    $("#origen_nombre").change(function () {
        changeInputString("nombre", $("#origen_nombre").val());
    });
    ...
    $("#btnBusqueda").click(function () {
        ...
        $.ajax({
            ...
            success: function (list) {
                $("#btnBusqueda").removeAttr("disabled");
                $("#tableOrigenes > tbody").empty();
                $("#tableOrigenes").footable({
                    "paging": {
                        "enabled": true
                    }
                });
                ...
                for (var i = 0; i < list.length; i++) {
                    ...
                    var ItemRow = '<tr data-expanded="true">' +
                        '<td>  ' + list[i].idOrigen + '</td>' +
                        '<td>  ' + list[i].codigo + '  </td>' +
                        '<td>  ' + list[i].nombre + '  </td>' +
                        '<td>' +
                        '<button type="button" class="' + list[i].idOrigen + ' btnEditarOrigen btn btn-primary ">Editar</button>' +
                        '</td>' +
                        '</tr>';

                    $("#tableOrigenes").append(ItemRow);
