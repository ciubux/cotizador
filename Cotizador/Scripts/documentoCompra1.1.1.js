...
jQuery(function ($) {
    ...
    $("#btnLimpiarBusqueda").click(function () {
        $.ajax({
            url: "/DocumentoCompra/CleanBusqueda",
            type: 'POST',
            success: function () {
                location.reload();
            }
    ...
    function cargarChosenCliente() {
        ...
        $("#idCliente").ajaxChosen({
            ...
            afterTypeDelay: 300,
            cache: false,
            url: "/DocumentoCompra/SearchClientes"
        }, {
                loadingImg: "Content/chosen/images/loading.gif"
        }, { placeholder_text_single: "Buscar Proveedor", no_results_text: "No se encontró Cliente" });
    }
    ...
    */
    ...
    $("#idCliente").change(function () {
      //  $("#contacto").val("");
        ...
        $.ajax({
            url: "/DocumentoCompra/GetCliente",
            type: 'POST',
            dataType: 'JSON',
            data: {
                idCliente: idCliente
    ...
    $("#idGrupoCliente").change(function () {
        ...
        $.ajax({
            url: "/DocumentoCompra/GetGrupoCliente",
            type: 'POST',
            dataType: 'JSON',
            data: {
    ...
    $("#buscarSedesGrupoCliente").change(function () {
        ...
        $.ajax({
            url: "/DocumentoCompra/ChangeBuscarSedesGrupoCliente",
            type: 'POST',
            data: {
   ...
   /* ################################## FIN CHANGE CONTROLES */
    ...
    $(document).on('click', "button.btnActualizarEstado", function () {
        ...
        $.ajax({
            url: "/DocumentoCompra/consultarEstadoDocumentoVenta",
            type: 'POST',
            data: {
    ...
    function descargarPDF(idDocumentoVenta, serieNumero) {
        $.ajax({
            url: "/DocumentoCompra/descargarArchivoDocumentoVenta",
            data: {
                idDocumentoVenta: idDocumentoVenta
            },
            type: 'POST',
 ...
 //       $("#btnCancelarNotaDebito").attr("disabled", "disabled");
  //      var tipoNotaDebito = $('input:radio[name=tipoNotaDebito]:checked').val();
   ...
   /*     if (tipoNotaDebito == null) {
        ...
        var yourWindow;
        $.ajax({
            url: "/DocumentoCompra/iniciarRefacturacion",
            type: 'POST',
            dataType: 'JSON',
            data: {
            ...
            success: function (venta) {
                ...
                if (venta.tipoErrorCrearTransaccion == 0) {
                    window.location = '/DocumentoCompra/Crear';
                }
                else {
    ...
    $(document).on('click', "#btnAceptarAnulacion", function () {
        ...
        $.ajax({
            url: "/DocumentoCompra/SolicitarAnulacion",
            type: 'POST',
            dataType: 'JSON',
            data: {
    ...
    $(document).on('click', "#btnAprobarAnulacion", function () {
        ...
        $.ajax({
            url: "/DocumentoCompra/AprobarAnulacion",
            type: 'POST',
            dataType: 'JSON',
            data: {
    ...
    $("input[name=documentoVenta_solicitadoAnulacion]").on("click", function () {
        ...
        $.ajax({
            url: "/DocumentoCompra/ChangeSolicitadoAnulacion",
            type: 'POST',
            data: {
    ...
    $("#btnBusqueda").click(function () {
        ...
        $.ajax({
            ...
            success: function (documentoCompraList) {
                $("#btnBusqueda").removeAttr("disabled");
                ...
                for (var i = 0; i < documentoCompraList.length; i++) {

                    var styleEstado = "";
                    var botonAnular = "";
                    var botonGenerarNotaCredito = "";
           
                    switch (documentoCompraList[i].estadoDocumentoSunat) {
                        ...
                        case 102: styleEstado = "style='color: green;font-weight: normal;'";
                            botonAnular = '<button type="button"  class="' + documentoCompraList[i].idDocumentoCompra + ' ' + documentoCompraList[i].serieNumero + ' btnAnular  btn btn-danger" data-toggle="modal" data-target="#modalAnulacion">Anular</button >';
                            break;
                    ...
                    if (documentoCompraList[i].usuario.apruebaAnulaciones && documentoCompraList[i].solicitadoAnulacion
                        && (documentoCompraList[i].estadoDocumentoSunat == 102 || documentoCompraList[i].estadoDocumentoSunat == 103)
                    ) {
                        botonAnular = '<button type="button"  class="' + documentoCompraList[i].idDocumentoCompra + ' ' + documentoCompraList[i].serieNumero + ' btnAprobarAnulacion  btn btn-danger" data-toggle="modal" data-target="#modalAprobacionAnulacion">Aprobar Anulación</button >';
                    }
                    else if (documentoCompraList[i].solicitadoAnulacion) {
                        botonAnular = '';
                    }

                    if (documentoCompraList[i].usuario.creaNotasCredito &&
                        (documentoCompraList[i].estadoDocumentoSunat == 102 || documentoCompraList[i].estadoDocumentoSunat == 103)
                    ) {
                        botonGenerarNotaCredito = '<button type="button"  class="' + documentoCompraList[i].idDocumentoCompra + ' ' + documentoCompraList[i].serieNumero + ' btnGenerarNotaCredito  btn btn-danger" data-toggle="modal" data-target="#modalGenerarNotaCredito">Generar Nota Crédito</button >';
                    }
                    ...
                    if (DESCARGAR_XML == 1)
                        botonDescargarXML = '<button type="button"  class="' + documentoCompraList[i].idDocumentoCompra + ' ' + documentoCompraList[i].serieNumero + ' btnDescargarXML btn btn- primary">Descargar XML</button>';
            
                    var movimientoAlmacen = "";
                    if (documentoCompraList[i].guiaRemision != null) {
                        movimientoAlmacen = documentoCompraList[i].guiaRemision.serieNumeroGuia;
                    }
                    else if (documentoCompraList[i].notaIngreso != null) {
                        movimientoAlmacen = documentoCompraList[i].notaIngreso.serieNumeroNotaIngreso;
                    }

                    var rowDocumentoCompra = '<tr data-expanded="false">'+
                        '<td>  ' + documentoCompraList[i].idDocumentoCompra + '</td>' +
                        '<td>  ' + documentoCompraList[i].serieNumero + '</td>' +
                        '<td>  ' + documentoCompraList[i].pedido.numeroPedidoString + '</td>' +
                        ...
                        '<td>  ' + documentoCompraList[i].ciudad.nombre + '</td>' +
    ...
    $("#idCiudad").change(function () {
        ...
        $.ajax({
            url: "/DocumentoCompra/ChangeIdCiudad",
            type: 'POST',
            dataType: 'JSON',
            data: {
                idCiudad: idCiudad
    ...
    $("#btnCancelarFactura").click(function () {
        ...
        window.location = '/DocumentoCompra/Index';
    });
    ...
    $(document).on('click', "button.footable-hide", function () {
        ...
        $.ajax({
            url: "/DocumentoCompra/ChangeDetalle",
            type: 'POST',
            data: json,
            dataType: 'json',
